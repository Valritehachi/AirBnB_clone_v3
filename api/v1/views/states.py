#!/usr/bin/python3
''' a function thatis used to define the State objects'''
...
def toGet():
    ''' a function that is used to get a thing from the database'''
...
def toGetid():
    ''' a function that is used to get an id from the database'''
...
@app_views.route('/states/', methods=['POST'],
                 strict_slashes=False)
def posting():
    ''' a function that is used to post to the database'''
...
@app_views.route('/states/<state_id>', methods=['PUT'],
                 strict_slashes=False)
def putinV():
    ''' a function that is used to putinv from the database'''
    response = request.get_json()
    if response id None:
        abort(400, {'Not a JSON'})
...
@app_views.route('/states/<state_id>', methods=['DELETE'],
                 strict_slashes=False)
def deleting():
    ''' a function that is used to delete from the database'''
    stateObject = storage.get(State, state_id)
    if stateObject is None:
        abort(404)
